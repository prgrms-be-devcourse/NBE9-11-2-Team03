...
public class FestivalAdminController {
    ...
    //목록 동기화 후, 변경된 contentId만 상세 보강까지 함께 수행한다.
    // TODOS: 추후 메인 관리자 동기화 메인 API로 협의 필요
    ...
    public RsData<FestivalSyncResponseDto> syncAndEnrichFestivals(
        ...
        List<String> changedContentIds = listResult.getChangedContentIds();
        ...
        if (changedContentIds == null || changedContentIds.isEmpty()) {
            ...
            return RsData.success("축제 목록 동기화가 완료되었고, 상세 보강 대상은 없습니다.", response);
        }
